...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.deepclaw/.env ] && grep -q "^GITHUB_TOKEN=" ~/.deepclaw/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.deepclaw/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from DeepClaw\",
    \"comments\": [
...
```bash
# Ensure gh auth is configured, or run the inline setup block from the top of this skill
```
...
gh pr review $PR_NUMBER --approve --body "Reviewed by DeepClaw. Code looks clean — good test coverage, no security concerns."
